...
std::string get_omop_id(std::unordered_map<std::string, std::unordered_map<std::string, std::string>> &omop_group_id_map,
    ...
    if(omop_id == "NA") {
        ...
        if(omop_group_id_map["LABfi"].find(omop_identifier) != omop_group_id_map["LABfi"].end()) {
            omop_id = omop_group_id_map["LABfi"][omop_identifier];
        } else if(omop_group_id_map["LABfi_HUS"].find(omop_identifier) != omop_group_id_map["LABfi_HUS"].end()) {
            ...
            omop_id = omop_group_id_map["LABfi_TMP"][omop_identifier];
        } else if(omop_group_id_map["LABfi_TKU"].find(omop_identifier) != omop_group_id_map["LABfi_TKU"].end()) {
            omop_id = omop_group_id_map["LABfi_TKU"][omop_identifier];
        } else {
            omop_id = "NA";
        }
    }
